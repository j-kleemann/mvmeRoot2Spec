...
class BinningSpec():
  ...
  def constructEdgeAlignedBinning(cls, upperEdge: float, binWidth: float, lowerEdge: float = 0, edgeAlignmentValue: float = 0, exceptionOnRounding: bool = True) -> BinningSpec:
    ...
    alignedLowerEdge = cls.alignToEdge(math.floor, lowerEdge, edgeAlignmentValue, binWidth)
    alignedUpperEdge = cls.alignToEdge(math.ceil, upperEdge, edgeAlignmentValue, binWidth)
  ...
  def __post_init__(self) -> None:
    ...
    if self.lowerEdge >= self.upperEdge:
      raise ValueError(f"Invalid BinningSpec: {self.lowerEdge=} >= {self.upperEdge=}!")
    upperEdge = self.alignToEdge(math.ceil, self.upperEdge, self.lowerEdge, self.binWidth)
  ...
  @classmethod
  def alignToEdge(cls, alignF: Callable[[float], float], val: float, edgeVal: float, binWidth: float) -> float:
    "Returns an aligned according to alignF (e.g. floor or ceil) value of val to match edgeVal in a quantization of binWidth."
    # Round is used to alleviate floating point math accuracy problems, the 10th decimal digit should really not be relevant to floor/ceil
    return cls.roundToSignificantFigures(alignF(round((val - edgeVal) / binWidth, 10)) * binWidth + edgeVal)

  @staticmethod
  def roundToSignificantFigures(val: float, sigs: int = 10) -> float:
    "Rounds val to sigs significant figures. Here used to alleviate unexpected results from floating point math inaccuracy."
...
class ScaleCalibration(TupleCalibration):
  ...
  timeBinWidth: Final = 25 / 1024 # 25 ns / 1024 was the used MDPP-16 TDC resolution, see also tdc_resolution in https://www.mesytec.com/products/datasheets/MDPP-16_SCP-RCP.pdf
  moduleTimestampCounterStepWidth: Final = 1 / 16 # in µs: The module_timestamp property counts the number of 16 MHz VME-clock signals received by the module since the start of the DAQ in a 30 bit integer, hence the width of a step is 1 / 16 MHz = 62.5 ns = 0.0625 µs
...
class ExportSetting:
  ...
  moduleTimestampIdentifier: str = "module_timestamp[1]"
  moduleTimestampChannel: ChannelNo = 0
  ...
  timeRawHistsRebinningFactors: Sequence[int] = (1, )
  ...
  moduleTimestampRawRange: int = 2**30
  moduleTimestampRawDiffHistsMaxVal: float = 5000
  moduleTimestampRawDiffHistsRebinningFactors: Sequence[int] = (1, )
  moduleTimestampCalDiffHistsMaxT: float = 2000 # in µs
  moduleTimestampCalDiffHistsBinWidths: Sequence[float] = (0.2, ) # in µs
  ...
  @property
  def moduleTimestampModElem(self) -> MvmeModuleElement:
    "Get the MvmeModuleElement for the module's event timestamps."
    return self.module + self.modulePropertyJoiner + self.moduleTimestampIdentifier
...
class Config:
  ...
  buildRawModuleTimestampDiffs: bool = True
  buildCalModuleTimestampDiffs: bool = True
  ...
  exportSettingsInitF: dataclasses.InitVar[Callable[[Config], Sequence[ExportSetting]]] = lambda cfg: [
    ExportSetting(module="clovers_up", channels=range(16), addbackAutoGroup=4, amplitudeSignalToRFTimeGateIntervalsDict={
      0: (990, 1100),
      1: (990, 1100),
      2: (990, 1100),
      3: (990, 1100),
    ...
    ExportSetting(
      module="clovers_down",
      channels=(0, 1, 2, 3, 7, 5, 6, 4) + ((15, ) if 715 < cfg.rootFilePathToRunNo() <= 744 else ()),
      addbackAutoGroup=4,
      amplitudeSignalToRFTimeGateIntervalsDict={
        0: (90, 170),
        1: (90, 170),
        2: (90, 170),
        3: (85, 165),
        7: (70, 150), # Remember: Erroneously swapped channels in the DAQ (ELOG ID 1884)
        5: (80, 160),
        6: (85, 165),
  ...
  def __post_init__(self, exportSettingsInitF) -> None:
    ...
    if self.mvmeModuleElements is None:
      self.mvmeModuleElements = set(mod for exSt in self.exportSettings for mod in (exSt.amplitudeModElem, exSt.channelTimeModElem, exSt.beamRFModElem, exSt.moduleTimestampModElem))
  ...
  def rootFilePathToPrefix(self) -> str:
    "Returns a prefix based on self.rootFilePath by striping/converting only known components and leaving the rest as is. Consults self.runNoPrefixDict for an optional additional suffix to the run number."
    prefix = os.path.basename(self.rootFilePath)
    if "." in prefix:
      prefix = prefix.rpartition(".")[0]

    def substitutionFunc(match: re.Match) -> str:
      "Helper function to return a replacement string consulting self.runNoPrefixDict for a match of a run number regex."
      runNoStr = match.group(1)
      runNo = int(runNoStr)
      return RUN_STR + runNoStr + ("_" + self.runNoPrefixDict[runNo] if runNo in self.runNoPrefixDict else "")

    return re.sub("mvmelst_(\\d+)_raw", substitutionFunc, prefix, count=1)
  ...
  def buildModuleTimestampConsecutiveDiffsHists(self, calibDictOrFalse: Union[Literal[False], CalibDict]) -> list[Hist1D]:
    "Builds and returns all consecutive module timestamp difference histograms from self.exportSettings either calibrated or raw according to calibDictOrFalse. The differences are taken modulo to the maximal timestamp value to handle counter overflows."
    hists = []
    for exSt in self.exportSettings:
      if calibDictOrFalse is False:
        for binWidth in exSt.moduleTimestampRawDiffHistsRebinningFactors:
          bSpec = BinningSpec.constructEdgeAlignedBinning(exSt.moduleTimestampRawDiffHistsMaxVal, binWidth, exceptionOnRounding=False)
          hists.append(Hist1D.constructConsecutiveDifferencesHist1D(self.outPrefix, exSt.moduleTimestampModElem, exSt.moduleTimestampChannel, bSpec, modulo=exSt.moduleTimestampRawRange, fillThreads=self.histFillThreads))
      else:
        for binWidth in exSt.moduleTimestampCalDiffHistsBinWidths:
          bSpec = BinningSpec.constructCenterAlignedBinning(exSt.moduleTimestampCalDiffHistsMaxT, binWidth, exceptionOnRounding=False)
          hists.append(Hist1D.constructConsecutiveDifferencesHist1D(self.outPrefix, exSt.moduleTimestampModElem, exSt.moduleTimestampChannel, bSpec, calibDictOrFalse, modulo=exSt.moduleTimestampRawRange * ScaleCalibration.moduleTimestampCounterStepWidth, fillThreads=self.histFillThreads))
    return hists
  ...
  def buildAllHists(self, calibDict: CalibDict) -> list[Hist1D]:
    ...
    if self.buildRawModuleTimestampDiffs:
      hists += self.buildModuleTimestampConsecutiveDiffsHists(False)
    if self.buildCalModuleTimestampDiffs:
      hists += self.buildModuleTimestampConsecutiveDiffsHists(calibDict)
    return hists
...
class DataAccumulatorBase(abc.ABC):
  ...
  timeCalibrations: ClassVar[dict[str, Calibration]] = {prop: ScaleCalibration(binWidth) for prop, binWidth in ((ExportSetting.beamRFIdentifier, ScaleCalibration.timeBinWidth), (ExportSetting.channelTimeIdentifier, ScaleCalibration.timeBinWidth), (ExportSetting.moduleTimestampIdentifier, ScaleCalibration.moduleTimestampCounterStepWidth))}
  ...
  def createHistNamePart(cls, modElem: MvmeModuleElement, label: str = DET_STR, channelNo: ChannelNo = 0, channelNoOffset: int = 1, propAppendix="") -> str:
    ...
    if prop == ExportSetting.beamRFIdentifier and channelNo == ExportSetting.beamRFSourceChannel:
      return f'{mod}_RF{propAppendix}'
    if prop == ExportSetting.moduleTimestampIdentifier and channelNo == ExportSetting.moduleTimestampChannel:
      return f'{mod}_ModTime{propAppendix}'
...
class Hist1D(DataAccumulatorBase):
  ...
  @classmethod
  def constructConsecutiveDifferencesHist1D(cls, prefix: str, mod: MvmeModuleElement, channelNo: ChannelNo, bSpec: BinningSpec, cal: Union[Literal[False], CalibDict, Calibration] = False, modulo: float = 0, histFlowBinsEnabled: bool = False, fillThreads: Optional[int] = None) -> Hist1D:
    "Classmethod to construct a difference histogram (Value-Previous_Value) of the consecutive (neighboring) non-NaN MvmeModuleElement's channel's events with appropriate name and dataProcessor. If `modulo` is non-zero the differences will be taken mod `modulo` to be able to handle counter overflows."
    name = cls.createHistName(prefix, mod, DET_STR, channelNo, cal is not False, bSpec.binWidth, propAppendix="ConsecutiveDifferences")
    if isinstance(cal, dict):
      cal = cls.findCalib(mod, channelNo, cal)
    lastValue = 0
    ...
    def dataProcessor(data: LazyCachedAugmentedModuleBatch) -> np.ndarray:
      nonlocal lastValue
      nonNaNData = data.getLazy(mod, channelNo, cal)[data.getLazy(mod, channelNo, "NonNaNMask")]
      diffs = np.diff(nonNaNData, prepend=lastValue) % modulo if modulo else np.diff(nonNaNData, prepend=lastValue)
      if len(nonNaNData) > 0:
        lastValue = nonNaNData[-1]
      return diffs

    return cls(name, bSpec, dataProcessor, fillThreads, histFlowBinsEnabled=histFlowBinsEnabled)

  @classmethod
  def constructSignalToRFTimeGatedHist1D(cls, prefix: str, mod: MvmeModuleElement, channelNo: ChannelNo, gateInterval: tuple[float, float], bSpec: BinningSpec, eCal: Union[Literal[False], CalibDict, Calibration] = False, tCals: Union[CalibDict, Tuple[Calibration, Calibration]] = {}, invertGate: bool = False, modBeamRF: MvmeModuleElement = None, channelNoBeamRF: ChannelNo = ExportSetting.beamRFSourceChannel, modTime: MvmeModuleElement = None, channelNoT: ChannelNo = None, gatePeriod: float = 179.21, fillThreads: Optional[int] = None) -> Hist1D:
    """Classmethod to construct a signal-time-to-RF-time-gated amplitude 1D histogram of an MvmeModuleElement's channel's raw or calibrated values with appropriate name and dataProcessor.
