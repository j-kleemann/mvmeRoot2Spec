...
rootFilePath = cliArgs.rootFilePath
inCalFilePath = cliArgs.inCalFilePath
outCalFilePath = cliArgs.outCalFilePath
outDir = cliArgs.outDir
rawHistRebinningFactors=cliArgs.rawHistRebinningFactors
calHistBinningSpecs=cliArgs.calHistBinningSpecs
exportRaw=cliArgs.exportRaw
exportCal=cliArgs.exportCal
exportAddback=cliArgs.exportAddback
outcalOverwrite=cliArgs.outcalOverwrite
...
def processAddbackDataBlock(dataID, channels, data) :
  ...
  for channelNo, channelData in zip(channels, data) :
    ...
    if calName not in calsDict :
      return
    channelData=calsDict[calName](channelData) # To optimize performance one could try reusing the result from the processDataBlock call...
...
with uproot.open(rootFilePath) as rootFile :
  for moduleDictBlock in rootFile['event0'].iterate(tuple(mvmeModulesChannelsDict), library="np", step_size="100 MB") :
...
if len(hists) > 0 :
  ...
  print("There were no spectra created! You might want to check your settings...?")
